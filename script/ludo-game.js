...
/*One or more token on same ceell then width and height of the tokens set from these functions*/
function getUpdatedWHoutAreaPos(noInId) {
  let posLength = outAreaPos[noInId].length;  //how many tokens at that position
  let wh = [];
  if (posLength > 0) {
    wh[0] = 100 / posLength;
    wh[1] = 100 / posLength;
...
function moveOnOutArea(cValue) {
  ...
  let timerId = setInterval(function () {
    ...
    if (count == rndmNo) {
      ...
      //Play safe cell sound only if landed on a safe cell
      const safeCells = [1, 9, 14, 22, 27, 35, 40, 48];
      if (safeCells.includes(noInId) && sound === true) {
        let safeStarAudio = new Audio("../music/safe-star-sound.mp3");
        safeStarAudio.play();
        setTimeout(() => {
          safeStarAudio.pause();
